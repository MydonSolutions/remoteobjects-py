...
class TestRemoteObject(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        defineRemoteClass(
            "Dummy", "http://localhost:6000", globals(), attribute_depth_allowance=-1
        )
    ...
    def test_method_filepath_upload(self):
        ...
        self.assertTrue(
            remoteDummy.file_contains_affirmative(script_dir + "/affirmative.txt")
        )
    ...
    def test_nested_access(self):
        remoteDummy = DummyRemote(dumbness="A tired subject")
        self.assertEqual(
            remoteDummy.internal_object.nested_object.str_attribute, "Nested"
        )
    ...
    def test_grandparent_access(self):
        ...
        self.assertEqual(
            remoteDummy.internal_object.nested_object.grandparent, remoteDummy
        )
        self.assertEqual(
    ...
    def test_grandparent_method(self):
        remoteDummy = DummyRemote(dumbness="That of a grandparent...")
        self.assertEqual(
            remoteDummy.internal_object.nested_object.grandparent.add(1, 1), 2
        )
...
if __name__ == "__main__":
    ...
    class Internal(object):
        def __init__(self, parent, string="Hello World!"):
            ...
            self.nested_object: Nested = Nested(string="Nested", grandparent=parent)
    ...
    app = Flask(__name__)
    addRemoteObjectResources(app, [Dummy])
    server_thread = threading.Thread(
        target=app.run,
        kwargs={"host": "0.0.0.0", "port": 6000, "debug": False},
        daemon=True,
    )
    server_thread.start()
    time.sleep(0.5)
