def _define_remote_constructor(
    ...
    definition_loc += [
        ...
        *[
        ...
        f"\t\t\t\t\t\tself._remote_object_id",
        f"\t\t\t\t\t)",
        f"\t\t\t\t)",
        f"\t\tfor (name, _) in response.json()['attributes'].items():",
        f"\t\t\tself._add_property(self._remote_object_id, name)",
        f"\t\tfor (name, _) in response.json()['attributes_nonbuiltins'].items():",
        f"\t\t\tsetattr(self, name, RemoteAttribute(",
        f"\t\t\t\tself._server_uri,",
        f"\t\t\t\tself._remote_object_id,",
        f"\t\t\t\tname,",
        f"\t\t\t\tallowed_upload_extension_regex",
        f"\t\t\t))",
        f"",
    ]
